...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        # Track last creation tool to allow toggling between edit and previous create mode
        self._last_create_mode = "rectangle"
        ...
        self.dirty = False
        # 지연 저장(auto_save_on_navigate) 모드에서 변경 사항이 있는지 추적
        ...
        self.canvas.brightness_contrast_request.connect(self.brightness_contrast_wheel_adjust)
        ...
        if False and self._config["auto_switch_to_edit_mode"]:  # noqa: E712  (강제 비활성화)
        ...
        # 빠른 편집 전용 단축키 (메뉴/툴바에 추가하지 않고 T 키로 바로 편집 모드 전환)
        # 사용자가 요구: 액션/메뉴 만들지 말고 단축키만. 기존 edit_polygon(Ctrl+J)은 유지하며 T 추가.
        self._quick_edit_shortcut = QtWidgets.QShortcut(
        ...
        brightness_contrast = action(
            ...
            None,
            "color",
            "Adjust brightness and contrast (Ctrl+Shift+Wheel for quick adjustment)",
            enabled=False,
        )
    ...
    def set_dirty(self):
        ...
        # 1) 지연 저장 모드: 실제 저장은 이미지 이동(open_next_image / open_prev_image 등) 직전에 수행
    ...
    def _flush_pending_auto_save(self):
        """지연 저장 플래그가 켜져 있을 때 실제 저장을 수행.

        - auto_save_on_navigate 모드 전용
        - flush 후 플래그 초기화
        - 예외는 사용자 워크플로우를 막지 않도록 로깅만
        """
        if not self._config.get("auto_save_on_navigate", False):
            return
        ...
        try:
            ...
            logger.warning(f"지연 자동 저장 실패: {e}")
        finally:
            self._pending_auto_save = False
    ...
    def update_file_status_info(self):
        if not getattr(self, "image_list", None) or not self.filename:
            return
        ...
        if current_index < 0:
            ...
            info = f"[{current_index + 1}/{len(self.image_list)}] {filename}"
        main_win = self.window()
        if hasattr(main_win, "set_top_file_index_text"):
            main_win.set_top_file_index_text(info)
    ...
    def toggle_draw_mode(
        self, edit: bool = True, create_mode: str | None = "rectangle", disable_auto_labeling: bool = True
    ):
        """편집(Edit) 모드 <-> 생성(Draw) 모드 전환.

        Sticky(create) 모드 동작을 위해 다음과 같이 변경:
        - 기존 구현: edit=True 로 들어올 때마다 canvas.create_mode 를 항상 'rectangle' 로 재설정.
        - 변경 후: create_mode 인자가 None 이면 현재 canvas.create_mode 를 그대로 유지 (즉, 마지막 선택 모드 유지).
        - edit=False (그리기 전환) 이면서 create_mode 가 명시되면 그때만 변경.

        매개변수
        edit: True -> 편집 모드, False -> 그리기 모드
        create_mode: 명시 시 해당 도형 모드로 변경. None 이면 유지. (호환성을 위해 기본값은 'rectangle' 이지만 edit=True 인 경우 자동 적용하지 않음)
        disable_auto_labeling: 모드 전환 시 자동 라벨링 비활성화 여부
        """
        ...
        # Sticky create_mode: 편집 모드 진입 시 강제 초기화하지 않고, create_mode 가 명시될 때만 변경
        if create_mode is not None and not edit:
            # 그리기 모드로 들어가며 새 모드 명시되었을 때만 변경
            self.canvas.create_mode = create_mode
        elif create_mode is not None and edit and self.canvas.create_mode is None:
            # 아주 이례적으로 None 인 경우만 기본값 보정
            self.canvas.create_mode = create_mode
        if edit:
            self.actions.create_mode.setEnabled(True)
            self.actions.create_rectangle_mode.setEnabled(True)
    ...
    def toggle_edit_create_mode(self):
        """Edit <-> Last create mode 토글 (요청: T 단축키로 모드 전환).

        - 현재 편집 중이면 마지막 사용 생성 모드(기본 rectangle)로 전환
        - 현재 그리기 중이면 편집 모드로 전환
        """
        # 마지막 생성 모드 기본값 보장
        if not hasattr(self, "_last_create_mode"):
            self._last_create_mode = "rectangle"
        if self.canvas.editing():
            create_mode = self._last_create_mode
            self.toggle_draw_mode(edit=False, create_mode=create_mode)
        else:
            # 기록 및 편집 복귀
            try:
                self._last_create_mode = self.canvas.create_mode
            except Exception:
                self._last_create_mode = "rectangle"
            self.toggle_draw_mode(edit=True)
        self.label_instruction.setText(self.get_labeling_instruction())
    ...
    def batch_edit_labels(self, shapes):
        ...
        text, flags, group_id, description, difficult, kie_linking, tag = result
    ...
    def _auto_save_generic(self):
        ...
        - 파일 다이얼로그 없이 동작
        - 기존 label_file 경로 재사용, 없으면 현재 이미지 basename + suffix
        - save_labels() 는 YOLO txt + 조건부 json 처리 포함
        """
        if not self.image_path:
            return
        try:
            if self.label_file and getattr(self.label_file, 'filename', None):
                target = self.label_file.filename
            else:
                base_no_ext, _ = osp.splitext(self.image_path)
                target = base_no_ext + LabelFile.suffix
            self.save_labels(target)
        except Exception as e:  # pragma: no cover
            logger.warning(f"Auto-save 실패: {e}")
    ...
    def save_labels(self, filename):
        # 기본: YOLO txt + classes.txt 동기화 (rectangle 기준)
        # 추가 요구사항: 이미지 전체 설명(other_data['description']) 또는 어떤 shape.description 이라도 존재할 때만 JSON 파일 별도 저장
        try:
            self._sync_yolo_annotation()
        except Exception as e:  # pragma: no cover
            logger.warning(f"YOLO 저장 중 오류: {e}")
            return False

        # YOLO 저장 성공 시 리스트 체크 표시
        try:
        ...
        # 조건 판단: 이미지 설명 또는 개별 객체 설명 존재 여부
        has_image_desc = bool(self.other_data.get("description", "").strip())
        has_shape_desc = any(
            (
                (getattr(s, "description", None) or "")
                or (getattr(s, "other_data", {}) or {}).get("description", "")
            ).strip()
    ...
    def brightness_contrast(self, _):
        self.brightness_contrast_dialog.update_image(
            image_utils.img_data_to_pil(self.image_data)
        )

        brightness, contrast = self.brightness_contrast_values.get(
            self.filename, (50, 50)  # Use consistent default values
        )
        if brightness is not None:
            ...
            self.brightness_contrast_dialog.slider_brightness.setValue(50)
        if contrast is not None:
            self.brightness_contrast_dialog.slider_contrast.setValue(contrast)
        else:
            self.brightness_contrast_dialog.slider_contrast.setValue(50)
    ...
    def brightness_contrast_wheel_adjust(self, delta, is_brightness):
        """Adjust brightness or contrast using mouse wheel with Ctrl+Shift"""
        if not hasattr(self, 'image_data') or self.image_data is None:
            return
            
        # Get current values or defaults (BrightnessContrastDialog uses 50 as neutral)
        brightness_contrast = self.brightness_contrast_values.get(self.filename, (50, 50))
        current_brightness = brightness_contrast[0] if brightness_contrast[0] is not None else 50
        current_contrast = brightness_contrast[1] if brightness_contrast[1] is not None else 50
        
        # Calculate adjustment step (smaller for finer control)
        step = 2 if delta > 0 else -2
        
        if is_brightness:
            # Adjust brightness (limit between 0 and 100, where 50 = 1.0)
            new_brightness = max(0, min(100, current_brightness + step))
            new_contrast = current_contrast
        else:
            # Adjust contrast (limit between 0 and 100, where 50 = 1.0)  
            new_brightness = current_brightness
            new_contrast = max(0, min(100, current_contrast + step))
        
        # Update values
        self.brightness_contrast_values[self.filename] = (new_brightness, new_contrast)
        
        # Apply the adjustment using the same logic as BrightnessContrastDialog
        import PIL.Image
        import PIL.ImageEnhance
        
        try:
            # Convert image data to PIL Image
            pil_image = image_utils.img_data_to_pil(self.image_data)
            
            # Apply brightness/contrast adjustment (convert slider values to enhancement factors)
            brightness_factor = new_brightness / 50.0  # 50 -> 1.0, 0 -> 0.0, 100 -> 2.0
            contrast_factor = new_contrast / 50.0
            
            img = pil_image
            if brightness_factor != 1:
                img = PIL.ImageEnhance.Brightness(img).enhance(brightness_factor)
            if contrast_factor != 1:
                img = PIL.ImageEnhance.Contrast(img).enhance(contrast_factor)
            
            # Convert back to QImage and update canvas
            qimage = image_utils.pil_to_qimage(img)
            self.on_new_brightness_contrast(qimage)
    ...
    def load_file(self, filename=None):  # noqa: C901
        ...
        try:
            ...
            if osp.exists(json_path):
                lf = LabelFile(filename=json_path)
                self.label_file = lf
                self.other_data = getattr(lf, 'other_data', {})
                shapes = lf.shapes
                if shapes:
                    self.load_shapes(shapes, update_last_label=False)
                    self.update_combo_box()
                    self.update_gid_box()
                # 이미지 설명 복원
                img_desc = self.other_data.get('description', '').strip()
                if img_desc:
                    try:
        ...
        # 2) JSON 없을 때만 YOLO fallback 로드
        if not json_loaded:
            try:
                import pathlib, importlib
                spec = importlib.util.spec_from_file_location(
                    'yolo_util_fallback',
                    str(pathlib.Path(__file__).resolve().parents[2] / 'utils' / 'yolo.py')
                )
                if not spec or not spec.loader:
                    raise ImportError('yolo.py spec 생성 실패')
                # fix: use importlib.util.module_from_spec (importlib.module_from_spec 없음)
                import importlib.util as _ilu
                m = _ilu.module_from_spec(spec)  # type: ignore
                spec.loader.exec_module(m)  # type: ignore
                load_yolo_annotations = m.load_yolo_annotations  # type: ignore
                load_yolo_classes = m.load_classes  # type: ignore
                img_path = pathlib.Path(filename)
                txt_path = img_path.with_suffix('.txt')
                classes_txt = img_path.parent / 'classes.txt'
                if txt_path.exists():
                    img_w, img_h = self.image.width(), self.image.height()
                    from .shape import Shape
                    shapes = []
                    if classes_txt.exists():
                        # 정상 경로: classes.txt 존재
                        classes = load_yolo_classes(classes_txt)
                        yolo_boxes = load_yolo_annotations(txt_path, img_w, img_h, classes)
                        for b in yolo_boxes:
                            x1, y1, x2, y2 = b['x1'], b['y1'], b['x2'], b['y2']
                            s = Shape(label=b['label'], shape_type='rectangle')
                            s.add_point(QtCore.QPointF(x1, y1))
                            s.add_point(QtCore.QPointF(x2, y1))
                            s.add_point(QtCore.QPointF(x2, y2))
                            s.add_point(QtCore.QPointF(x1, y2))
                            s.close()
                            shapes.append(s)
                    else:
                        # fallback: classes.txt 없지만 txt 만 있는 기존 데이터
                        # YOLO 형식 직접 파싱: class cx cy w h (normalized)
                        try:
                            ...
                            for line in lines:
                                ...
                                x2 = min(img_w, x_center + bw / 2)
                                y2 = min(img_h, y_center + bh / 2)
                                s = Shape(label=str(cls_id), shape_type='rectangle')
                                s.add_point(QtCore.QPointF(x1, y1))
                                s.add_point(QtCore.QPointF(x2, y1))
                                s.add_point(QtCore.QPointF(x2, y2))
                                s.add_point(QtCore.QPointF(x1, y2))
                ...
                logger.warning(f"YOLO txt 로드 실패: {e}")
        ...
        brightness, contrast = self.brightness_contrast_values.get(
            self.filename, (50, 50)  # Use safe defaults
        )
        if self._config["keep_prev_brightness"] and self.recent_files:
            prev_brightness, _ = self.brightness_contrast_values.get(
                self.recent_files[0], (50, 50)
            )
            brightness = prev_brightness if prev_brightness is not None else 50
        if self._config["keep_prev_contrast"] and self.recent_files:
            _, prev_contrast = self.brightness_contrast_values.get(
                self.recent_files[0], (50, 50)
            )
            contrast = prev_contrast if prev_contrast is not None else 50
        ...
        self.brightness_contrast_dialog.slider_contrast.setValue(contrast)
        self.brightness_contrast_values[self.filename] = (brightness, contrast)
        # Always call on_new_value since we have valid values now
    ...
    def save_file_dialog(self):
        ...
        # NOTE: Windows 네트워크(UNC) 경로에서 네이티브 파일 다이얼로그 사용 시
        # QWindowsNativeFileDialogBase::shellItem SHCreateItemFromParsingName 관련 오류가
        # 간헐적으로 발생(특히 file:// 또는 잘못된 토큰 경로 변환 케이스). 안정성을 위해 비네이티브 다이얼로그 강제.
    ...
    def get_label_file(self):
        # Option A: description 있을 때 생성된 JSON 이 존재하면 그 JSON을 우선 사용, 없으면 YOLO txt 경로 반환
    ...
    def delete_file(self):
        ...
        # JSON 삭제 로직 제거 (.txt 는 사용자가 직접 관리)
        label_file = self.get_label_file()
        if osp.exists(label_file) and label_file.endswith('.json'):
            try:
    ...
    def remove_selected_point(self):
        ...
        self.canvas.update()
        # 포인트 삭제로 형상 변경 시 dirty 처리 (shape 자체 삭제 여부 무관)
        self.set_dirty()
        if self.canvas.h_hape is not None and not self.canvas.h_hape.points:
            ...
            # shape 사라진 경우도 set_dirty 이미 호출됨. 저장 정책은 set_dirty 로 위임
            if self.no_shape():
                for action in self.actions.on_shapes_present:
                    action.setEnabled(False)
    ...
    def import_dropped_image_files(self, image_files):
        ...
        for file in image_files:
            ...
            if (QtCore.QFile.exists(yolo_file)) or (
    ...
    # classes.txt 관리
    # ------------------------------------------------------------------
    def _ensure_classes_in_dir(self, dirpath: str):
        """지정 폴더에 classes.txt 없거나 비어있으면 사용자 입력을 받아 생성.

    - json 파일은 생성하지 않음(요청사항).
    - 실패/취소 시 아무것도 하지 않음.
        """
        # utils.py(모듈) vs utils/ 디렉토리 충돌 회피 위해 직접 경로 import
        import importlib, pathlib
        spec = importlib.util.spec_from_file_location(
            'classes_util_fallback',
            str(pathlib.Path(__file__).resolve().parents[2] / 'utils' / 'classes.py')
        )
        if not spec or not spec.loader:  # pragma: no cover
            logger.warning("classes util import 실패 (spec 생성 실패)")
            return
        m = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(m)  # type: ignore
        read_classes_txt = m.read_classes_txt  # type: ignore
        write_classes_txt = m.write_classes_txt  # type: ignore

        import pathlib
        p = pathlib.Path(dirpath)
        txt_path = p / 'classes.txt'
        classes = read_classes_txt(txt_path)
        if classes:  # 이미 존재 & 내용 있음 -> json 동기화 필요 없음 (요청: json 생성 X)
            # convert_txt_to_json(txt_path)  # 비활성화
            return

        # 사용자에게 생성/불러오기 방식 선택
        choice_box = QMessageBox(self)
        choice_box.setWindowTitle(self.tr('클래스 정의 필요'))
        choice_box.setText(self.tr('선택한 폴더에 classes.txt 가 없습니다. 생성 방법을 선택하세요.'))
        btn_input = choice_box.addButton(self.tr('직접입력'), QMessageBox.AcceptRole)
        btn_file = choice_box.addButton(self.tr('파일선택'), QMessageBox.ActionRole)
        btn_cancel = choice_box.addButton(self.tr('취소'), QMessageBox.RejectRole)
        choice_box.exec_()
        ...
        if clicked == btn_file:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as rf:
                    raw_lines = [l.strip() for l in rf.readlines()]
            except Exception as e:  # pragma: no cover
                QMessageBox.warning(self, self.tr('오류'), self.tr(f'파일 읽기 실패: {e}'))
                return
            raw = []
            for line in raw_lines:
                if not line:
                    continue
                # 콤마가 포함된 라인은 분리 허용
                parts = [seg.strip() for seg in line.replace(',', '\n').split('\n')]
                raw.extend(parts)
            seen = set()
            for c in raw:
            ...
            text, ok = QtWidgets.QInputDialog.getMultiLineText(
                self,
                self.tr('클래스 입력'),
                self.tr('클래스 이름을 줄바꿈 또는 콤마로 구분하여 입력하세요:'),
            )
            if not ok or not text.strip():
                return
            raw = [seg.strip() for line in text.splitlines() for seg in line.replace(',', '\n').split('\n')]
            seen = set()
            for c in raw:
                if not c or c in seen:
                    continue
        ...
        if not final:
            QMessageBox.information(self, self.tr('알림'), self.tr('유효한 클래스가 없습니다.'))
            return
        # 여기까지 오면 최종 클래스 목록 확정
        try:
            write_classes_txt(txt_path, final)
            logger.info(
                f"classes.txt 생성( json 미생성 ): {txt_path} ({len(final)} classes)"
            )
        except Exception as e:  # pragma: no cover
            QMessageBox.warning(self, self.tr('오류'), self.tr(f'classes.txt 저장 실패: {e}'))
            logger.warning(f"classes.txt 저장 실패: {e}")
    ...
    # YOLO txt 동기화
    # ------------------------------------------------------------------
    def _sync_yolo_annotation(self):  # noqa: C901 (단순 로직, 길이 제한 무시)
        """현재 라벨(사각형) 정보를 YOLO txt로 저장 및 classes.txt 업데이트.

        규칙:
        - rectangle shape 만 대상 (다른 타입은 스킵)
        - classes.txt 없으면 현재 라벨 순서대로 생성
        - 기존 classes.txt 에 없던 새 라벨은 append 후 재작성
        - AutoLabelingMode 특수 라벨 제외
        - 오류 발생 시 raise 하지 않고 상위에서 잡아 로그 처리
        """
        if not self.image_path:
            return
        import pathlib, importlib
        ...
        classes = read_classes_txt(classes_path) if classes_path.exists() else []
        updated = False
        existing = set(classes)

        # 수집 대상 shape -> rectangle
        boxes = []
        for item in self.label_list:
            shape = item.shape()
            if not shape or shape.label in [
                AutoLabelingMode.OBJECT,
                AutoLabelingMode.ADD,
                AutoLabelingMode.REMOVE,
            ]:
            ...
            if label not in existing:
                ...
                existing.add(label)
                updated = True
            # rectangle 은 4점 (좌상,우상,우하,좌하)
            try:
                x1 = min(p.x() for p in shape.points)
                y1 = min(p.y() for p in shape.points)
                x2 = max(p.x() for p in shape.points)
                y2 = max(p.y() for p in shape.points)
            except Exception:
                continue
            boxes.append({
                'label': label,
        ...
        # classes.txt 필요 시 생성/갱신
        if not classes_path.exists() or updated:
            try:
                write_classes_txt(classes_path, classes)
            except Exception as e:  # pragma: no cover
