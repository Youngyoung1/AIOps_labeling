...
class Canvas(
    ...
    brightness_contrast_request = QtCore.pyqtSignal(int, bool)  # delta, is_brightness
    ...
    def mouseDoubleClickEvent(self, _):
        ...
        Added behavior: when in editing mode and a shape is under cursor (hovered),
        open the label edit dialog (same as list item double-click).
        """
        if self.is_loading:
            return

        # New: open label dialog in editing mode
        if self.editing() and self.h_hape is not None:
            # Ensure the shape is selected so parent logic works consistently.
            if self.h_hape not in self.selected_shapes and self.h_hape.points:
                # Select using first vertex point
                try:
                    self.select_shape_point(self.h_hape.points[0], False)
                except Exception:  # noqa: BLE001 - fail-safe, keep UX smooth
                    pass
            # Invoke parent's edit label method (same as list double click)
            try:
    ...
    def wheelEvent(self, ev: QWheelEvent):
        ...
        # Check for Ctrl+Shift+Wheel for brightness/contrast adjustment
        if (QtCore.Qt.ControlModifier & int(mods)) and (QtCore.Qt.ShiftModifier & int(mods)):
            # Ctrl+Shift+Wheel for brightness (vertical scroll) or contrast (horizontal scroll)
            if abs(delta.y()) > abs(delta.x()):
                # Vertical scroll - brightness
                self.brightness_contrast_request.emit(delta.y(), True)
            else:
                # Horizontal scroll - contrast
                self.brightness_contrast_request.emit(delta.x(), False)
            ev.accept()
            return
