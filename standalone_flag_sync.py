...
try:
    ...
    print("❌ pymongo가 설치되지 않음. pip install pymongo 실행하세요.")
    sys.exit(1)
...
class SimpleMongoAnnotationManager:
    """간단한 MongoDB Annotation 매니저"""
    
    def __init__(self):
        try:
            # MongoDB 연결
            self.client = MongoClient('mongodb://localhost:27017/')
            self.db = self.client['x_anylabeling']
...
class SimpleFlagSync:
    ...
    def __init__(self):
        self.annotation_manager = SimpleMongoAnnotationManager()
        self.collection = self.annotation_manager.collection
    ...
    def mark_all_for_sync(self, directory_filter: str = None):
        ...
        try:
            query = {}
            if directory_filter:
                # Windows 경로를 MongoDB 경로로 변환
                normalized_path = directory_filter.replace('\\', '/')
                query['json_file_path'] = {'$regex': normalized_path}
    ...
    def mark_specific_files(self, file_patterns: list):
        ...
        for pattern in file_patterns:
            try:
                ...
                query = {'json_file_path': {'$regex': normalized_pattern}}
    ...
    def sync_flagged_to_json(self, batch_size: int = 10):
        ...
        try:
            # 플래그된 문서들 조회
            flagged_docs = list(self.collection.find({
                self.sync_flag: True
            }).limit(batch_size))
    ...
    def _sync_single_mongodb_to_json(self, mongodb_doc: dict, json_file_path: str) -> bool:
        ...
        try:
            ...
            # MongoDB 데이터로 업데이트
            if mongodb_doc.get('flags') != json_data.get('flags', {}):
                json_data['flags'] = mongodb_doc.get('flags', {})
                changes_made = True
                
            if mongodb_doc.get('shapes') != json_data.get('shapes', []):
                json_data['shapes'] = mongodb_doc.get('shapes', [])
                changes_made = True
                
            if mongodb_doc.get('description') != json_data.get('description', ''):
            ...
            if changes_made:
                ...
                print(f"📝 변경사항 저장됨: {os.path.basename(json_file_path)}")
            else:
                print(f"📊 변경사항 없음: {os.path.basename(json_file_path)}")
    ...
    def get_flagged_files(self, limit: int = 10):
        """플래그된 파일 목록 반환"""
        try:
            docs = list(self.collection.find(
                {self.sync_flag: True},
                {'json_file_path': 1, 'sync_marked_time': 1}
            ).limit(limit))
    ...
    def show_detailed_status(self):
        ...
        try:
            ...
            flagged_count = self.get_flagged_count()
            ...
            if flagged_count > 0:
                print(f"\n🚩 플래그된 파일 목록 (최대 10개):")
                flagged_files = self.get_flagged_files(10)
                
                for i, doc in enumerate(flagged_files, 1):
                    file_path = doc.get('json_file_path', 'Unknown')
                    filename = os.path.basename(file_path) if file_path else 'Unknown'
                    marked_time = doc.get('sync_marked_time')
                    
                    if marked_time:
                        if isinstance(marked_time, str):
                            time_str = marked_time
                        else:
                            time_str = marked_time.strftime('%H:%M:%S')
                    else:
                        time_str = 'Unknown'
                        
                    print(f"   {i:2d}. {filename} (마킹: {time_str})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="간단한 플래그 기반 동기화 도구",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
사용 예시:
  %(prog)s status                    # 현재 상태 확인
  %(prog)s mark-all -d "C:\\test"    # 특정 디렉토리 모든 파일 플래그 설정
  %(prog)s sync                      # 플래그된 파일들 동기화
  %(prog)s clear                     # 모든 플래그 제거
  %(prog)s update-test               # 테스트용 MongoDB 업데이트
        """
    )
    ...
    try:
        ...
        if args.action == 'mark-all':
            ...
            sync_tool.show_detailed_status()
