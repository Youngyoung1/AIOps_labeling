"""
MongoDB 설정을 단일 JSON(mongo_config.json)에서 로드하고
MongoStorage 인스턴스를 싱글턴으로 제공하는 프로바이더.

사용:
    from anylabeling.services.storage.mongo_provider import get_storage, get_config_value
    mongo = get_storage()
    limit = get_config_value('limits', 'aggregation_limit', default=1000)
"""
import json
import os
from typing import Any, Optional

from .mongodb_client import MongoStorage

_CONFIG_CACHE: Optional[dict] = None
_STORAGE_SINGLETON: Optional[MongoStorage] = None


def _config_path() -> str:
    # 현재 파일 기준 동일 디렉터리의 mongo_config.json
    return os.path.join(os.path.dirname(__file__), 'mongo_config.json')


def load_config(force_reload: bool = False) -> dict:
    global _CONFIG_CACHE
    if _CONFIG_CACHE is not None and not force_reload:
        return _CONFIG_CACHE
    path = _config_path()
    try:
        with open(path, 'r', encoding='utf-8') as f:
            _CONFIG_CACHE = json.load(f)
    except Exception:
        # 기본값 폴백
        _CONFIG_CACHE = {
            "uri": os.getenv("MONGODB_URI", "mongodb://localhost:27017/"),
            "db_name": "labeling_db",
            "collections": {
                "annotations": "annotations",
                "images": "images",
                "flags": "flags",
            },
            "embedding": {"enabled": True, "model": "all-MiniLM-L6-v2"},
            "limits": {"aggregation_limit": 1000},
        }
    return _CONFIG_CACHE

def get_config_value(*keys: str, default: Any = None) -> Any:
    cfg = load_config()
    cur: Any = cfg
    for k in keys:
        if isinstance(cur, dict) and k in cur:
            cur = cur[k]
        else:
            return default
    return cur

def get_storage() -> MongoStorage:
    global _STORAGE_SINGLETON
    if _STORAGE_SINGLETON is not None:
        return _STORAGE_SINGLETON

    cfg = load_config()
    uri = cfg.get("uri") or os.getenv("MONGODB_URI", "mongodb://localhost:27017/")
    db_name = cfg.get("db_name", "labeling_db")
    collections = cfg.get("collections", {})
    annotations_col = collections.get("annotations", "annotations")
    images_col = collections.get("images", "images")
    flags_col = collections.get("flags", "flags")

    # 임베딩 사용 여부 반영 (mongodb_client는 환경변수 DISABLE_VECTOR_SEARCH로 제어 가능)
    embedding_cfg = cfg.get("embedding", {})
    enabled = embedding_cfg.get("enabled", True)
    model_name = embedding_cfg.get("model", "all-MiniLM-L6-v2")

    if not enabled:
        os.environ["DISABLE_VECTOR_SEARCH"] = "1"
    else:
        # 명시적으로 0으로 재설정 (다른 곳에서 1로 둔 경우 해제)
        if os.environ.get("DISABLE_VECTOR_SEARCH") == "1":
            os.environ["DISABLE_VECTOR_SEARCH"] = "0"

    # 1차 시도
    _STORAGE_SINGLETON = MongoStorage(
        uri=uri,
        db_name=db_name,
        annotations_collection=annotations_col,
        images_collection=images_col,
        flags_collection=flags_col,
        embedding_model=model_name,
    )
    try:
        if not _STORAGE_SINGLETON.test_connection():
            raise RuntimeError("initial connection failed")
    except Exception:
        # localhost가 IPv6(::1)로만 바인딩되지 않은 환경에서 실패할 수 있으므로 IPv4로 폴백
        if "localhost" in (uri or ""):
            ipv4_uri = uri.replace("localhost", "127.0.0.1")
            _STORAGE_SINGLETON = MongoStorage(
                uri=ipv4_uri,
                db_name=db_name,
                annotations_collection=annotations_col,
                images_collection=images_col,
                flags_collection=flags_col,
                embedding_model=model_name,
            )
    return _STORAGE_SINGLETON
# -*- coding: utf-8 -*-
"""
MongoDB 단일 프로바이더 모듈
- JSON 설정(mongo_config.json)로 모든 Mongo 연결/옵션 관리
- 기존 MongoStorage(mongodb_client.py)를 감싸서 공통 진입점을 제공
사용 예:
    from anylabeling.services.storage.mongo_provider import get_storage
    storage = get_storage()  # 싱글톤
    storage.find_annotations({...})
"""
import json
import os
import threading
from typing import Optional

from .mongodb_client import MongoStorage

_CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'mongo_config.json')
_lock = threading.Lock()
_instance: Optional[MongoStorage] = None
_config_cache = None


def load_config(path: Optional[str] = None):
    global _config_cache
    cfg_path = path or _CONFIG_PATH
    if _config_cache is not None:
        return _config_cache
    with open(cfg_path, 'r', encoding='utf-8') as f:
        _config_cache = json.load(f)
    return _config_cache


def get_storage(force_reload: bool = False, config_path: Optional[str] = None) -> MongoStorage:
    """공용 MongoStorage 인스턴스 획득(싱글톤)
    Args:
        force_reload: True이면 설정을 다시 읽고 인스턴스를 재생성
        config_path: 기본 경로 대신 다른 설정 파일 경로 사용
    """
    global _instance, _config_cache
    with _lock:
        if force_reload:
            _instance = None
            _config_cache = None
        if _instance is None:
            cfg = load_config(config_path)
            coll = cfg.get('collections', {})
            embedding_cfg = cfg.get('embedding', {})
            _instance = MongoStorage(
                uri=cfg.get('uri'),
                db_name=cfg.get('db_name', 'labeling_db'),
                annotations_collection=coll.get('annotations', 'annotations'),
                images_collection=coll.get('images', 'images'),
                flags_collection=coll.get('flags', 'flags'),
                embedding_model=embedding_cfg.get('model', 'all-MiniLM-L6-v2'),
            )
            # 임베딩 비활성화 옵션
            if not embedding_cfg.get('enabled', True):
                _instance.embedding_model = None
        return _instance


def get_config_value(*keys, default=None):
    cfg = load_config()
    cur = cfg
    for k in keys:
        if not isinstance(cur, dict) or k not in cur:
            return default
        cur = cur[k]
    return cur
