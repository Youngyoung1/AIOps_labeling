...
class LabelDialog(QtWidgets.QDialog):
    def __init__(
        ...
        # Add Tag section with checkboxes
        self.tag_options = {
            "썬팅": QtWidgets.QCheckBox("썬팅"),
            "태양빛 반사": QtWidgets.QCheckBox("태양빛 반사"),
            "장애물": QtWidgets.QCheckBox("장애물"),
        ...
        for i, (tag_name, checkbox) in enumerate(self.tag_options.items()):
            pass  # No button group needed for checkboxes
        ...
        # difficult checkbox
        layout.addWidget(self.edit_difficult)
        
        # Tag section
        tag_label = QtWidgets.QLabel(self.tr("Tag"))
        tag_label.setStyleSheet("font-weight: bold; margin-top: 10px;")
        layout.addWidget(tag_label)
        
        # Add tag radio buttons
        for tag_name, radio_btn in self.tag_options.items():
    ...
    def get_tag_state(self):
        selected_tags = []
        for tag_name, checkbox in self.tag_options.items():
            if checkbox.isChecked():
                selected_tags.append(tag_name)
        return selected_tags if selected_tags else []  # Return list of selected tags or empty list
    ...
    def pop_up(
        ...
        # Set tag checkboxes selection
        # First, uncheck all checkboxes
        for checkbox in self.tag_options.values():
            checkbox.setChecked(False)
        
        # Then check the ones that should be selected
        if tag and isinstance(tag, list):
            for tag_name in tag:
                if tag_name in self.tag_options:
                    self.tag_options[tag_name].setChecked(True)
        elif tag and isinstance(tag, str) and tag in self.tag_options:
            self.tag_options[tag].setChecked(True)
        self.edit.setText(text)
