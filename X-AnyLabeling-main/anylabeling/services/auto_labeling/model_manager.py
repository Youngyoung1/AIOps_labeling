...
class ModelManager(QObject):
    ...
    def load_model_configs(self):
        ...
        for model in model_list:
            ...
            if config_file.startswith(":/"):  # 리소스(qrc) 내 기본 모델 설정
                config_file_name = config_file[2:]
                try:
                    # Traversable.joinpath 는 한 번에 한 세그먼트만 허용 → 체이닝 필요
                    resource_base = pkg_resources.files(auto_labeling_configs).joinpath("auto_labeling")
                    resource_path = resource_base.joinpath(config_file_name)
                    with open(resource_path, "r", encoding="utf-8") as f:
                        model_config = yaml.safe_load(f)
                        model_config["config_file"] = str(config_file)
                except Exception as e:  # pragma: no cover
                    logger.error(
                        "[ModelManager] Failed to load config for '%s': %s. Entry skipped.",
                        model.get("name", config_file_name),
                        e,
                    )
                    continue
