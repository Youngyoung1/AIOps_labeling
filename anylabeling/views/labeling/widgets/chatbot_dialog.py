...
try:
    ...
    # Provide a helpful error at runtime when the openai package is not available.
    # This keeps import-time checks quiet but gives a clear message if the code
    # later tries to instantiate the OpenAI client without the package installed.
    class OpenAI:
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "The 'openai' package is required for ChatbotDialog functionality. "
                "Install it with 'pip install openai' or configure a provider that does not require it."
            )
